...
class AdminDir {
  ...
  *_addRepository(packageName, arch, distribution, targetRoot, next) {
  ...
  _makeSource(uri, distribution, components) {
    let source = `wpkg ${uri
      .replace(/\+/g, '%2B')
  ...
  *addSource(uri, arch, distribution, components, next) {
    const repo = getDebRoot(distribution, this._resp).replace(/\\/g, '/');
    ...
    const source = this._makeSource(uri, distribution, components);
    yield this._wpkg.addSources(source, arch, targetRoot, next);
  }

  *delSource(uri, arch, distribution, components, next) {
    const repo = getDebRoot(distribution, this._resp).replace(/\\/g, '/');
