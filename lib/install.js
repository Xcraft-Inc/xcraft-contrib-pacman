...
class Install {
  ...
  *package(packageRef, prodRoot, reinstall, next) {
    ...
    const res = yield this._admindir.create(packageRef, prodRoot);
    yield this._wpkg.install(res.name, res.arch, prodRoot, reinstall, next);
  }

  *status(packageRef, next) {
    const pkg = utils.parsePkgRef(packageRef);

    const isInstalled = yield this._wpkg.isInstalled(pkg.name, pkg.arch, next);
    this._response.log.info(
