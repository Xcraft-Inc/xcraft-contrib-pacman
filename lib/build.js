...
class Build {
  ...
  static _parseDeps(deps) {
    return deps.split(', ').reduce((state, dep) => {
      ...
      const [_, name, version, archs] = matches;
      state[name] = {
        version: version || '',
  ...
  *_getPkgDef(packageName, arch, distribution, next) {
    return Build._pkg2def(
      (yield this._publish.status(packageName, distribution, null))
  ...
  *_buildDeps(packageList, arch) {
    ...
    packageList.forEach((dep) => {
      // FIXME: extract from the package (in the repository) instead of the
      //        Xcraft definition.
  ...
  *_publishInstallDeps(
    ...
    for (const pkg of packageList) {
      ...
      // FIXME: extract from the package (in the repository) instead of the
      //        Xcraft definition.
      const def = definition.load(packageName, null, this._resp, distribution);
