...
class Build {
  ...
  *_isSrcPublished(packageName, distribution) {
    const srcPackageName = `${packageName}-src`;
    if (!this._pkgSrcPublishStatus[srcPackageName]) {
      this._pkgSrcPublishStatus[srcPackageName] = yield this._publish.status(
        srcPackageName,
        distribution,
        null
      );
    }
    return this._pkgSrcPublishStatus[srcPackageName];
  }

  *_getPkgDef(packageName, arch, distribution, next) {
    packageName = packageName.replace(/-dev$/, '');
    return Build._pkg2def(
      (yield this._isSrcPublished(packageName, distribution))
        ? yield this._wpkg.show(
            `${packageName}-src`,
            arch,
  ...
  *_tryBuild(packageName, arch, distribution, topPackage, next) {
    ...
    if (packageDef.architecture.indexOf('source') === -1) {
      ...
      for (const pkg of Build._installList(
        ...
        if (this._triedList[key]) {
          continue;
        }
        this._triedList[key] = true;
  ...
  *package(packageRef, distribution, next) {
    ...
    try {
      this._triedList = {};
      this._pkgSrcPublishStatus = {};
      yield this._resp.command.send('overwatch.clear-all-errors', null, next);
      ...
      utils.errorReporting(this._resp)(data);
    }
  }
}
