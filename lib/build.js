...
class Build {
   ...
   */
  * _buildDeps (packageName, arch, response, next) {
    ...
    for (const pkg of list) {
      if (pkg.type === 'bin') {
        yield this._tryBuild (pkg.name, arch, false, response);
        continue;
      }
   ...
   * @param {string} packageName
   */
  _getBuildDeps (packageName, response) {
    const packageDef = definition.load (packageName, null, response);
   ...
   * @param {function(err)} next
   */
  * _installBuildDeps (packagelist, arch, response, next) {
    try {
   ...
   */
  * _getInstallDeps (packageName, arch, response) {
    const packageDef = definition.load (packageName, null, response);
    let list = {};
   ...
   */
  * _publishInstallDeps (packageName, packageList, arch, outputRepository, response, next) {
    let isEmpty = true;
    const list = [];
   ...
   */
  _buildSrc (packageName, arch, repository, response, callback) {
   ...
   * @param {function(err)} next
   */
  * _unpublishStub (list, repository, response, next) {
   ...
   * @param {function(err)} next
   */
  * _build (packageName, arch, topPackage, response, next) {
    ...
    try {
      ...
      yield this._buildDeps (packageName, arch, response);

      /* Step 2 */
      response.log.info (`get build dependencies of ${packageName}`);
      const buildDepsList = this._getBuildDeps (packageName, response);

      /* Step 3 */
      response.log.info (`install new build dependencies`);
      yield this._installBuildDeps (buildDepsList, arch, response);
      ...
      const resDeps = yield this._publishInstallDeps (startPackage, packageList, arch, outputRepository, response);

      /* Step 6 */
      if (!resDeps.skip) {
        response.log.info (`build the src repository`);
        yield this._buildSrc (null, arch, outputRepository, response, next);
      }

      /* Step 7 */
      if (resDeps.list.length) {
        response.log.info (`unpublish stub packages`);
        yield this._unpublishStub (resDeps.list, null, response);
      }
    } catch (ex) {
      throw ex;
    } finally {
  ...
  * _tryBuild (packageName, arch, topPackage, response) {
    const packageDef = definition.load (packageName, null, response);
    ...
    if (packageDef.architecture.indexOf ('source') === -1) {
      ...
      for (const pkg of Object.keys (packageDef.dependency.install)) {
        yield this._tryBuild (pkg, arch, false, response);
      }
    } else {
      /* It's a source dependency, begins the whole build. */
      yield this._build (packageName, arch, topPackage, response);
    }
  }
  ...
  package (packageRef, response, callback) {
    ...
    admindir.create (packageRef, response, (err, packageName, arch) => {
      ...
      this._tryBuild (packageName, arch, true, response, callback);
    });
  }
}
