...
exports.package = function (packageRef, callback) {
  ...
  xLog.verb ('compile package name: ' + (pkg.name || 'all') + ' on architecture: ' + pkg.arch);
  ...
  admindir.create (packageRef, function (err, packageName, arch) {
    ...
    /* TODO: add handling to build all source packages. */
    // wpkg.buildFromSrc (null, arch, 'toolchain/', callback);
  });
};
