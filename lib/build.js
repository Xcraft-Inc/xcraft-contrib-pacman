...
class Build {
   ...
   */
  * buildDeps (packageName, arch, response, next) {
    ...
    for (const pkg of list) {
      if (pkg.type === 'bin') {
        yield this.tryBuild (pkg.name, arch, false, response);
        continue;
      }
   ...
   * @param {string} packageName
   */
  getBuildDeps (packageName, response) {
    const packageDef = definition.load (packageName, null, response);
   ...
   * @param {function(err)} next
   */
  * installBuildDeps (packagelist, arch, response, next) {
    try {
   ...
   */
  * getInstallDeps (packageName, arch, response) {
    const packageDef = definition.load (packageName, null, response);
    let list = {};
    ...
    for (const dep of Object.keys (packageDef.dependency.install)) {
      ...
      list[dep] = true;
      const newList = yield this.getInstallDeps (dep, arch);
   ...
   */
  * publishInstallDeps (packageName, packageList, arch, outputRepository, response, next) {
    let isEmpty = true;
    const list = [];
   ...
   */
  buildSrc (packageName, arch, repository, response, callback) {
   ...
   * @param {function(err)} next
   */
  * unpublishStub (list, repository, response, next) {
   ...
   * @param {function(err)} next
   */
  * build (packageName, arch, topPackage, response, next) {
    ...
    try {
      ...
      yield this.buildDeps (packageName, arch, response);

      /* Step 2 */
      response.log.info (`get build dependencies of ${packageName}`);
      const buildDepsList = this.getBuildDeps (packageName, response);

      /* Step 3 */
      response.log.info (`install new build dependencies`);
      yield this.installBuildDeps (buildDepsList, arch, response);

      /* Step 4 */
      response.log.info (`get install dependencies of ${packageName}`);
      const installDepsList = Object.keys (yield this.getInstallDeps (packageName, arch));
      ...
      const resDeps = yield this.publishInstallDeps (startPackage, packageList, arch, outputRepository, response);

      /* Step 6 */
      if (!resDeps.skip) {
        response.log.info (`build the src repository`);
        yield this.buildSrc (null, arch, outputRepository, response, next);
      }

      /* Step 7 */
      if (resDeps.list.length) {
        response.log.info (`unpublish stub packages`);
        yield this.unpublishStub (resDeps.list, null, response);
      }
    } catch (ex) {
      throw ex;
    } finally {
  ...
  * tryBuild (packageName, arch, topPackage, response) {
    const packageDef = definition.load (packageName, null, response);
    ...
    if (packageDef.architecture.indexOf ('source') === -1) {
      ...
      for (const pkg of Object.keys (packageDef.dependency.install)) {
        yield this.tryBuild (pkg, arch, false, response);
      }
    } else {
      /* It's a source dependency, begins the whole build. */
      yield this.build (packageName, arch, topPackage, response);
    }
  }

  package (packageRef, response, callback) {
    ...
    admindir.create (packageRef, response, (err, packageName, arch) => {
      if (err) {
        callback (err);
        return;
      }

      this.tryBuild (packageName, arch, true, response, callback);
    });
  }
}
