...
class Build {
  ...
  *_buildDeps(packageList, arch) {
  ...
  _getBuildDeps(packageName, arch) {
    ...
    const buildList = buildDeps =>
      Object.keys(buildDeps).filter(name =>
        buildDeps[name].some(
          dep => !dep.architecture.length || dep.architecture.includes(arch)
        )
      );

    let list = buildList(packageDef.dependency.build);

    for (const dep of Object.keys(packageDef.dependency.install)) {
      const depDef = definition.load(dep, null, this._response);
      list = list.concat(buildList(depDef.dependency.build));
    }
