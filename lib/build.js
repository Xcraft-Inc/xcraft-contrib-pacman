...
class Build {
  ...
  *_installBuildDeps(packagelist, arch, next) {
    try {
      for (const pkg of packagelist) {
         ...
         * amount of time.
         */
        const isInstalled = yield this._install.status(pkg);
        if (!isInstalled) {
          yield this._install.package(pkg, null, false, next);
        }
      }
    } catch (ex) {
      throw ex;
  ...
  *_unpublishStub(list, repository) {
    ...
    yield this._publish.removeAll(list, repository);
  }
  ...
  *_build(packageName, arch, topPackage) {
    ...
    try {
      ...
      const installDepsList = Object.keys(
        yield this._getInstallDeps(packageName, arch)
      );
      ...
      for (const pkg of srcPackages) {
        try {
          const isInstalled = yield this._install.status(pkg);
          if (isInstalled) {
            yield this._remove.package(pkg, false);
          }
        } catch (ex) {
  ...
  *package(packageRef) {
    ...
    const res = yield this._admindir.create(packageRef, null);
