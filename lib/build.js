...
class Build {
  constructor(response) {
    ...
    const pacmanConfig = require('xcraft-core-etc')(null, this._response).load(
    ...
    this._remove = require('./remove.js')(response);
  ...
  *_installBuildDeps(packagelist, arch, distribution, next) {
    try {
      for (const pkg of packagelist) {
         ...
         * amount of time.
         */
        const isInstalled = yield this._install.status(pkg, null);
        if (!isInstalled) {
  ...
  *_getInstallDeps(packageName, arch, distribution) {
    ...
    for (const dep of Object.keys(packageDef.dependency.install)) {
      const depDef = definition.load(dep, null, this._response, distribution);
  ...
  *_build(packageName, arch, distribution, topPackage) {
    const xcraftConfig = require('xcraft-core-etc')(null, this._response).load(
      'xcraft'
    );
    ...
    try {
      ...
      for (const pkg of srcPackages) {
        try {
          const isInstalled = yield this._install.status(pkg, distribution);
          if (isInstalled) {
            yield this._remove.package(pkg, distribution, false);
          }
        } catch (ex) {
          this._response.log.warn(ex);
      ...
      try {
        ...
        if (!resDeps.skip) {
          this._response.log.info(
            `build the src repository for ${distribution}`
          );
  ...
  *package(packageRef, distribution) {
    ...
    if (distribution !== this._devDistribution) {
      yield this._admindir.create(
        packageRef,
        getTargetRoot(distribution, this._response),
        distribution
      );
    }
