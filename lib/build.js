...
function publishInstallDeps (packageName, packageList, arch, outputRepository, callback) {
  ...
  publish.add (packageName, null, outputRepository, (err) => {
    if (err) {
      callback (err);
      return;
    }
...
function build (packageName, arch, callback) {
  async.auto ({
    ...
    publishInstallDeps: ['getInstallDeps', (callback, res) => {
      ...
      publishInstallDeps (`${packageName}-src`, packageList, arch, outputRepository, (err, list) => {
        if (err) {
          callback (err);
          return;
        }
...
function tryBuild (packageName, arch, callback) {
  const packageDef = definition.load (packageName);
...
exports.package = function (packageRef, callback) {
  ...
  admindir.create (packageRef, function (err, packageName, arch) {
    ...
    tryBuild (packageName, arch, callback);
  });
};
