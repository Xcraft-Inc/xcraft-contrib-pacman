...
class Build {
  ...
  * _buildDeps (packageName, arch, next) {
    ...
    for (const pkg of list) {
      ...
      /* Only for 'src' type. */
      const deb = yield publish.status (pkg.name, null, this._response, next);
      if (!deb) {
        yield this._tryBuild (pkg.name, arch, false);
      }
    }
  ...
  * _installBuildDeps (packagelist, arch, next) {
    try {
      for (const pkg of packagelist) {
        yield install.package (pkg, false, this._response, next);
      }
    } catch (ex) {
      throw ex;
    } finally {
  ...
  * _publishInstallDeps (packageName, packageList, arch, outputRepository, next) {
    ...
    if (packageName) {
      yield publish.add (packageName, null, outputRepository, this._response, next);
      isEmpty = false;
    }
    ...
    for (const pkg of packageList) {
      ...
      // FIXME: handle the package version
      const deb = yield publish.status (packageName, null, this._response, next);
      if (!deb) {
        yield publish.add (pkg, null, outputRepository, this._response, next);
        isEmpty = false;
        continue;
      }
  ...
  * _unpublishStub (list, repository, next) {
    for (const stub of list) {
      yield publish.remove (`${stub}-stub`, repository, this._response, next);
    }
  }
  ...
  * _tryBuild (packageName, arch, topPackage) {
  ...
  package (packageRef, callback) {
    ...
    admindir.create (packageRef, this._response, (err, packageName, arch) => {
      if (err) {
        callback (err);
        return;
      }

      this._tryBuild (packageName, arch, true, callback);
    });
  }
}
