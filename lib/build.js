 ...
 */
const buildDeps = watt (function * (packageName, arch, next) {
  ...
  for (const pkg of list) {
    if (pkg.type === 'bin') {
      yield tryBuild (pkg.name, arch, false);
      continue;
    }

    /* Only for 'src' type. */
    const deb = yield publish.status (pkg.name, null, next);
    if (!deb) {
      yield tryBuild (pkg.name, arch, false);
    }
  }
 ...
 * @param {function(err)} next
 */
const installBuildDeps = watt (function * (packagelist, arch, next) {
  try {
    for (const pkg of packagelist) {
      yield install.package (pkg, false, next);
    }
  } catch (ex) {
    throw ex;
  } finally {
 ...
 */
const getInstallDeps = watt (function * (packageName, arch) {
  const packageDef = definition.load (packageName);
  let list = {};
 ...
 * @param {function(err)} next
 */
const unpublishStub = watt (function * (list, repository, next) {
  for (const stub of list) {
    yield publish.remove (`${stub}-stub`, repository, next);
  }
});
...
const build = watt (function * (packageName, arch, topPackage, next) {
  ...
  try {
    ...
    const resDeps = yield publishInstallDeps (startPackage, packageList, arch, outputRepository);

    /* Step 6 */
    if (!resDeps.skip) {
      xLog.info (`build the src repository`);
      yield buildSrc (null, arch, outputRepository, next);
    }

    /* Step 7 */
    if (resDeps.list.length) {
      xLog.info (`unpublish stub packages`);
      yield unpublishStub (resDeps.list);
    }
  } catch (ex) {
    throw ex;
  } finally {
...
tryBuild = watt (function * (packageName, arch, topPackage) {
  const packageDef = definition.load (packageName);
  ...
  if (packageDef.architecture.indexOf ('source') === -1) {
    ...
    for (const pkg of Object.keys (packageDef.dependency.install)) {
      yield tryBuild (pkg, arch, false);
    }
  } else {
    /* It's a source dependency, begins the whole build. */
    yield build (packageName, arch, topPackage);
  }
});
