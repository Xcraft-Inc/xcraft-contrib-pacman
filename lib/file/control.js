 ...
 */
var defToControl = function (packageDef, response) {
  var controlMap = {
    name:            'Package',
  ...
  packageDef.architecture.forEach (function (arch) {
    ...
    response.log.verb (util.format ('Control file (%s):\n%s', arch, control));
  });
 ...
 */
exports.controlFiles = function (packageArch, packageDef, saveFiles, response) {
  if (saveFiles) {
    response.log.info ('if necessary, save the control files for ' + packageDef.name);
  }

  var xcraftConfig = require ('xcraft-core-etc') (null, response).load ('xcraft');
  var pacmanConfig = require ('xcraft-core-etc') (null, response).load ('xcraft-contrib-pacman');
  var xFs          = require ('xcraft-core-fs');
  var utils        = require ('../utils.js');

  var fs = require ('fs');

  var control = defToControl (packageDef, response);
  var controlFiles = [];
  ...
  Object.keys (control).forEach (function (arch) {
    ...
    if (saveFiles) {
      if (fs.existsSync (controlFile)) {
        response.log.warn ('the control file will be overwritten: ' + controlFile);
      }
