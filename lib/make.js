...
class Make {
  ...
  *_bumpIfNecessary(packageName, version, distribution) {
    /* XXX: I think that, maybe, we can remove this stuff (notInWork)
     * because we want a bump only if the bin package is not already
     * built.
     */
  ...
  *package(packageName, arch, defProps, outputRepository, distribution) {
    ...
    if (!isStub) {
      ...
      bump = yield this._bumpIfNecessary(packageName, version, distribution);
      if (bump) {
        loadDefs();
      }
    } else {
