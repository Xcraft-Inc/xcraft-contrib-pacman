...
var createConfigJson = function (packageName, packageDef, postInstDir) {
  var xUri = require ('xcraft-core-uri');

  var config = packageDef.data;

  config.get.uri = xUri.realUri (config.get.uri, packageName);
...
var processFile = function (packageName, files, arch, packageDef, callback) {
  var i = 0;
  ...
  var nextFile = function () {
    ...
    var wpkgBuild = function (packageDef) {
      ...
      createConfigJson (packageName, packageDef, sharePath);
    ...
    var build = function (callback) {
      ...
      if (packageDef.data.embedded && packageDef.data.get.uri.length) {
        ...
        getObj.uri = xUri.realUri (utils.injectThisPh (packageDef, packageDef.data.get.uri), packageName);
        ...
        xPeon[dataType][rulesType] (getObj, packagePath, sharePath, extra, function (err) {
    ...
    async.auto ({
      ...
      taskBuildPackage: ['taskPostpeon', 'taskCopyPatches', function (err, results) {
...
exports.package = function (packageName, arch, defProps, callback) {
  ...
  /* Overload properties accordingly to defProps. */
  Object.keys (defProps).forEach (function (prop) {
    var path = prop.split ('.');
    if (traversed.has (path)) {
      xLog.verb ('overload property %s with the value %s', prop, defProps[prop]);
      traversed.set (path, defProps[prop]);
    }
  });
  ...
  try {
    var controlFiles = control.controlFiles (packageName, arch, packageDef, true);
    ...
    changelog.changelogFiles (packageName, arch, packageDef);
    copyright.copyrightFiles (packageName, arch, packageDef);
    cmakelists.cmakelistsFile (packageName, arch, packageDef);
    etc.etcFiles (packageName, arch, packageDef);

    processFile (packageName, controlFiles, arch, packageDef, callback);
  } catch (err) {
    callback (err);
  }
};
