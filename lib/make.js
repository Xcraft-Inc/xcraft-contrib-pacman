...
class Make {
  ...
  _copyTemplateFiles (def, packagePath, script, sharePath, sysRoot, isSource, response) {
  ...
  _build (packageDef, packagePath, sharePath, response, callback) {
  ...
  _wpkgBuild (packageDef, packagePath, sharePath, outputRepository, response, callback) {
    const pacmanConfig = require ('xcraft-core-etc') (null, response).load ('xcraft-contrib-pacman');
    ...
    if (packageDef.architecture.indexOf ('all') === -1) {
      ...
      scripts.forEach ((it) => {
        this._copyTemplateFiles (packageDef, packagePath, it.script, sharePath, it.sysRoot, isSource, response);
      });
    }
  ...
  _processFile (files, packageDef, useStamps, outputRepository, response, callback) {
    ...
    async.auto ({
      ...
      taskPeon: ['taskPrepeon', (callback) => {
        this._build (packageDef, packagePath, sharePath, response, callback);
      }],

      taskPostpeon: ['taskPeon', (callback) => {
      ...
      taskBuildPackage: ['taskPostpeon', 'taskCopyPatches', (callback) => {
        this._wpkgBuild (packageDef, packagePath, sharePath, outputRepository, response, (err) => {
      ...
      taskEnd: ['taskBuildPackage', (callback) => {
        callback ();
      }]
    }, (err) => {
      ...
      if (!files.length) {
        ...
        this._processFile (files, packageDef, useStamps, outputRepository, response, callback);
      }
    });
  }
  ...
  package (packageName, arch, defProps, outputRepository, callback) {
    ...
    try {
      ...
      this._processFile (controlFiles, packageDef, useStamps, outputRepository, this._response, callback);
    } catch (err) {
      callback (err);
    }
  }
