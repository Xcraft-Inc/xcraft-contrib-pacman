...
class Make {
  ...
  *package(packageName, arch, defProps, outputRepository, distribution) {
    ...
    const explicitDistrib = distribution && distribution.indexOf('+') !== -1;
    let packageDefs;
    let packageDef;
    ...
    const computeSum = () =>
      xFs.shasum(pkgDir, /^[^.]/) +
      (explicitDistrib ? distribution : '').trim();
    ...
    if (!isStub) {
      if (!outputRepository) {
        ...
        try {
          if (readStamp) {
            stamp = fse.readFileSync(stampFile).toString().trim();
      ...
      if (deb) {
        ...
        bump = yield this._bumpIfNecessary(packageName, deb.version);
        if (bump) {
          loadDefs();
        }
      }
