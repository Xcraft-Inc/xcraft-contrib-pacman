...
class Make {
  ...
  _processFile (files, packageDef, useStamps, outputRepository, callback) {
    ...
    async.auto ({
      ...
      taskPeon: ['taskPrepeon', (callback) => {
        this._build (packageDef, packagePath, sharePath, callback);
      }],
      ...
      taskBuildPackage: ['taskPostpeon', 'taskCopyPatches', (callback) => {
        this._wpkgBuild (packageDef, packagePath, sharePath, outputRepository, (err) => {
      ...
      if (!files.length) {
        ...
        this._processFile (files, packageDef, useStamps, outputRepository, callback);
      }
    });
  }
