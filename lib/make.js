...
var injectThisPh = function (packageDef, data) {
...
var createConfigJson = function (packageName, postInstDir) {
  ...
  fs.writeFileSync (outFile, injectThisPh (def, data), 'utf8');
};
  ...
  var nextFile = function () {
    ...
    var build = function (callback) {
      ...
      if (packageDef.data.embedded && packageDef.data.get.uri.length) {
        ...
        getObj.uri = xUri.realUri (injectThisPh (packageDef, packageDef.data.get.uri), packageName);
         ...
         */
        var extra = {
          configure:     injectThisPh (packageDef, packageDef.data.configure),
          location:      injectThisPh (packageDef, packageDef.data.rules.location),
...
exports.package = function (packageName, arch, callback) {
  ...
  try {
    ...
    changelog.changelogFiles (packageName, arch);
    copyright.copyrightFiles (packageName, arch);
    cmakelists.cmakelistsFile (packageName, arch);
    etc.etcFiles (packageName, arch);
