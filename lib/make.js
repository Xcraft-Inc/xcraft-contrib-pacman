...
var processFile = function (packageName, files, arch, callback) {
  ...
  var nextFile = function () {
    ...
    async.auto ({
      ...
      taskPeon: ['taskPremake', function (callback) {
        build (callback);
      }],
      taskPostmake: ['taskPeon', function (callback) {
        try {
          var postmake = require (path.join (productPath, 'postmake.js')) (packagePath, sharePath);
          postmake.copy (callback);
        } catch (err) {
          /* FIXME: how to handle the case where an internal require fails? */
          if (err.code === 'MODULE_NOT_FOUND') {
            xLog.info ('no postmake script for this package');
            callback ();
          } else {
            xLog.err (err);
            callback (err);
      ...
      taskBuildPackage: ['taskPostmake', function (err, results) {
