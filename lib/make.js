...
class Make {
  ...
  *_deployMakeDep(dep, next) {
    ...
    if (deb) {
      ...
      /* The dependency exists, we must just install this package. */
      const cmdMsg = {packageRefs: dep};
      const result = yield this._resp.command.send(
        'pacman.install',
        cmdMsg,
        next
      );
  ...
  *package(packageName, arch, defProps, outputRepository) {
    ...
    /* Case where toolchain+stub is used for creating new stub packages
