...
class Make {
  ...
  *_processFile(files, packageDefs, distribution, outputRepository, next) {
    ...
    try {
      ...
      /* 2. Copy assets */
      const assetsIn = path.join(productPath, 'assets');
      const assetsOut = path.join(sharePath, 'assets');
      if (fse.existsSync(assetsIn)) {
        xFs.cp(assetsIn, assetsOut);
      }
  ...
  *_bumpIfNecessary(packageName, distribution, targetDistribution) {
  ...
  injectBaseHash(packageName, hash) {
    const packageDef = this._def.load(packageName, {}, this._resp, null);
    if (!packageDef.data || !packageDef.data.get) {
      return;
    }

    this._resp.log.info(`Refresh the ${packageName} shasum to ${hash}`);
    this._def.baseUpdate(packageName, {'data.get.$hash': hash}, this._resp);
  }
  ...
  *_deployMakeDep(dep, next) {
    ...
    yield fullpac(this._resp, dep, true, null);
  }
  ...
  _computeStampSums(packageDir, distribution) {
    ...
    const globalDir = path.basename(packageDir).split(/[+-]/);
    const assetsGlobalDir = path.join(
      this._xcraftConfig.pkgProductsRoot,
      `.${globalDir[0]}`,
      'assets',
      globalDir[1]
    ...
    if (fse.existsSync(assetsGlobalDir)) {
      dirs.push(assetsGlobalDir);

      list = list.concat(
        xFs
          .lsall(assetsGlobalDir, false, (item) => !regex.test(item))
          .map((entry) => path.relative(assetsGlobalDir, entry))
      );
    }
    ...
    const listing = xUtils.crypto.sha256(list.join(':'));
  ...
  *_checkVersion(
    ...
    const comp = yield debversion(latestVersion, defVersion); // V1, V2
    const isLatestGreater = comp > 0;
   ...
   */
  async package(packageName, arch, defProps, outputRepository, distribution) {
    ...
    const targetDistribution = distribution; /* distribution for which the package is built */
    ...
    if (!isStub) {
      ...
      await this._admindir.create(`${packageName}:${arch}`, null, null);
      ...
      if (deb && packageDef.version !== deb.version) {
        ...
        const comp = await debversion(packageDef.version, deb.version);
        defGreaterThatDeb = comp > 0;
      }
      ...
      if (skip && (!deb || defGreaterThatDeb)) {
        ...
        for (const arch of packageDef.architecture) {
          try {
            deb = false;
            await this._wpkg.copyFromArchiving(
              packageName + suffix,
              arch,
              packageDef.version,
              distribution
            );
            deb = await this._publish.status(
              packageName + suffix,
              distribution,
              null
            );
      ...
      bump = await this._bumpIfNecessary(
        packageName,
        distribution,
        targetDistribution
      );
      if (bump) {
        ...
        return await this.package(...arguments);
      }
    } else {
      this._resp.log.warn(
    ...
    const {ref, hash} = await this._processFile(
      controlFiles,
      packageDefs,
      distribution,
      outputRepository
