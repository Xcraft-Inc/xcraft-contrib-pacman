...
var copyTemplateFiles = function (def, packagePath, script, sharePath, sysRoot, isSource) {
...
var wpkgBuild = function (packageDef, packagePath, sharePath, outputRepository, callback) {
  ...
  if (packageDef.architecture.indexOf ('all') === -1) {
    ...
    scripts.forEach (function (it) {
      copyTemplateFiles (packageDef, packagePath, it.script, sharePath, it.sysRoot, isSource);
    });
  }
  ...
  wpkg[action] (packagePath, packageDef.distribution, outputRepository, callback);
};

var processFile = function (files, packageDef, useStamps, outputRepository, callback) {
  ...
  async.auto ({
    ...
    taskBuildPackage: ['taskPostpeon', 'taskCopyPatches', function (callback) {
      wpkgBuild (packageDef, packagePath, sharePath, outputRepository, function (err) {
    ...
    if (!files.length) {
      ...
      processFile (files, packageDef, useStamps, outputRepository, callback);
    }
  });
};
 ...
 */
exports.package = function (packageName, arch, defProps, outputRepository, callback) {
  ...
  try {
    ...
    processFile (controlFiles, packageDef, useStamps, outputRepository, callback);
  } catch (err) {
    callback (err);
  }
};
