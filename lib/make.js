...
class Make {
  ...
  _copyTemplateFiles (def, packagePath, script, sharePath, sysRoot, isSource) {
    ...
    const ext    = isSource ? xPlatform.getShellExtArray () : [xPlatform.getShellExt ()];

    ext.forEach ((fileExt) => {
      const scriptFileIn  = path.join (path.join (__dirname, './templates/'),
                                       this._pacmanConfig.pkgScript + fileExt);
      const scriptFileOut = path.join (packagePath,
      ...
      ph.set ('NAME',    def.name)
        ...
        .set ('SHARE',   path.relative (packagePath, sharePath).replace (/\\/g, '\\\\'))
        .set ('HOOK',    'local')
        .set ('ACTION',  action)
        .set ('SYSROOT', sysRoot.replace (/\\/g, '\\\\'))
        .set ('CONFIG',  path.join (sysRoot, 'etc/peon.json').replace (/\\/g, '\\\\'))
  ...
  * _build (packageDef, packagePath, sharePath, next) {
    ...
    yield xPeon[dataType][rulesType] (getObj, packagePath, sharePath, extra, this._response, next);
  }
  ...
  * _wpkgBuild (packageDef, packagePath, sharePath, outputRepository, next) {
    ...
    const action = packageDef.architecture.indexOf ('source') === -1 ? 'build' : 'buildSrc';
    yield this._wpkg[action] (packagePath, packageDef.distribution, outputRepository, next);
  }
  ...
  * _processFile (files, packageDef, useStamps, outputRepository, next) {
    ...
    const exceptions = [];
    const productPath = path.join (this._xcraftConfig.pkgProductsRoot, packageDef.name);

    try {
      /* 1. Prepeon */
      const prepeonPath = path.relative (__dirname, path.join (productPath, 'prepeon.js'));
      try {
        delete require.cache[require.resolve (prepeonPath)];
        const prepeon = require (prepeonPath) (packagePath, sharePath, packageDef, this._response);
      ...
      /* 3 Postpeon */
      const postpeonPath = path.relative (__dirname, path.join (productPath, 'postpeon.js'));
      try {
        const postpeon = require (postpeonPath) (packagePath, sharePath, packageDef, this._response);
      ...
      if (useStamps) {
        const stampsDir = path.join (this._xcraftConfig.xcraftRoot, this._pacmanConfig.stamps);
  ...
  * package (packageName, arch, defProps, outputRepository) {
    ...
    const controlFiles = control.controlFiles (arch, packageDef, true, this._response);
    ...
    yield this._processFile (controlFiles, packageDef, useStamps, outputRepository);
  }
}
