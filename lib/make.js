...
class Make {
  ...
  *_deployMakeDep(dep, next) {
   ...
   */
  *_tryMake(packageName, arch, defProps, outputRepository) {
   ...
   */
  *package(packageName, arch, defProps, outputRepository, next) {
    try {
      yield this._resp.command.send('overwatch.clear-all-errors', null, next);
      yield this._tryMake(packageName, arch, defProps, outputRepository);
    } finally {
