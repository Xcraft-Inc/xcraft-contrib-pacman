...
var WpkgArgs = function (callback) {
  ...
  return {
    ...
    install: function (packagePath, arch) {
      ...
      args = args.concat (addRepositories (arch));
      args.push ('--install');
    ...
    listIndexPackages: function (repositoryPath, arch, listOut) {
      ...
      var regex = new RegExp ('(' + arch + '|all|source)');

      run (args, path.join (repositoryPath, pacmanConfig.pkgIndex), function (line) {
        var result = line.trim ().match (/.* ([^ _]*)_([^ _]*)(?:_([^ _]*))?\.ctrl$/);

        var deb = result[1] + '_' + result[2];

        /* Ignore packages from other architectures. */
        if (result[3]) {
          if (!regex.test (result[3])) {
            return;
          }
...
var lookForPackage = function (packageName, archRoot, callbackResult) {
...
exports.buildFromSrc = function (packageName, arch, distribution, callback) {
  ...
  lookForPackage (packageName, arch, function (deb) {
    if (!deb) {
