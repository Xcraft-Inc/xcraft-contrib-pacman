...
var WpkgArgs = function (callback) {
  ...
  return {
    ...
    listIndexPackages: function (repositoryPath, arch, listOut) {
      ...
      var regex = new RegExp ('(' + arch + '|all)');

      run (args, path.join (repositoryPath, pacmanConfig.pkgIndex), function (line) {
        var result = line.trim ().match (/.* (?:(.*)\/)?([^ _]*)_([^ _]*)(?:_([^ _]*))?\.ctrl$/);

        var debDistrib = result[1];
        var debName    = result[2];
        var debVersion = result[3];
        var debArch    = result[4];

        var deb = '';
        if (debDistrib) {
          deb = debDistrib + '/';
        }
        deb += debName + '_' + debVersion;

        /* Ignore packages from other architectures. */
        if (debArch) {
          if (!regex.test (debArch)) {
            return;
          }
...
exports.createAdmindir = function (arch, callback) {
  ...
  var fileIn  = path.join (__dirname, '../templates/control.admin');
