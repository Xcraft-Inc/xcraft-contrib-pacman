...
exports.update = function (packageName, props, resp, distribution) {
  ...
  exports.save(data, pkgConfig, resp);
};

function cleanDef(packageDef) {
  /* Do not keep empty sunpackage list (it looks like not a src package) */
  ...
  for (const depType of ['install', 'build', 'make']) {
    if (packageDef.dependency[depType]) {
      if (Object.keys(packageDef.dependency[depType]).length === 0) {
        ...
        delete packageDef.rules.args.makeall;
        delete packageDef.rules.args.maketest;
        delete packageDef.rules.args.makeinstall;
        delete packageDef.rules.test;
        delete packageDef.deploy;
...
exports.save = function (packageDef, pkgConfig, resp) {
  ...
  if (!pkgConfig) {
    pkgConfig = getPackageDefPath(packageDef.name, null, resp);
  }
