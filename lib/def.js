...
function getPackageDefPath(packageName, distribution, resp) {
  ...
  const pkgConfigUnstage = path.join(
    xcraftConfig.pkgProductsRoot,
    packageName,
    '.' + pacmanConfig.pkgCfgFileName
  );
  ...
  return {
    base: pkgConfig,
    unstage: pkgConfigUnstage,
    distrib: pkgDistribConfig,
  };
}
...
exports.getBasePackageDef = function (packageName, resp) {
  ...
  for (let i = 2; i > 0; --i) {
    ...
    const pkgConfigs = getPackageDefPath(packageName, null, resp);

    try {
      pkgDef = utils.yaml.fromFile(pkgConfigs.base);
      if (fs.existsSync(pkgConfigs.unstage)) {
        const unstageDef = utils.yaml.fromFile(pkgConfigs.unstage);

        if (unstageDef.version && unstageDef.version !== pkgDef.version) {
          /* Keep non-async, debversion must be already initialized */
          const comp = debversion.sync(
            `${unstageDef.version}`,
            `${pkgDef.version}`
          );
          if (comp < 0) {
            /* Unstaged version seems lower, the base version has been updated */
            resp.log.warn(
              `Keep the higher version of ${packageName}: ${pkgDef.version}, unstaged:${unstageDef.version}`
            );
            delete unstageDef.version;
            delete unstageDef.$version;
          }
        }

        pkgDef = overloadProps(pkgDef, flatten(unstageDef), null);
      }
      break;
    } catch (ex) {
      if (ex.code === 'ENOENT') {
...
exports.baseUpdate = function (packageName, props, resp) {
  packageName = packageName.replace(/(?:-stub|-src)$/, '');

  const pkgConfigs = getPackageDefPath(packageName, null, resp);
  const unstageDef = fs.existsSync(pkgConfigs.unstage)
    ? utils.yaml.fromFile(pkgConfigs.unstage)
    : {};
  const baseDef = utils.yaml.fromFile(pkgConfigs.base);

  delete unstageDef?.data?.get?.$hash;
  utils.yaml.toFile(unstageDef, pkgConfigs.unstage);

  const data = overloadProps(baseDef, props, resp);
...
exports.update = function (packageName, props, resp, distribution) {
  ...
  const packageDef = fs.existsSync(pkgConfigs.unstage)
    ? utils.yaml.fromFile(pkgConfigs.unstage)
    : {};
  ...
  exports.save(data, pkgConfigs.unstage, resp);
};
...
exports.removeUnstage = function (packageDef, resp) {
  const unstage = getPackageDefPath(packageDef.name, null, resp).unstage;
  xFs.fse.removeSync(unstage);
};
