...
exports.remove = function (packageRef, repository, callback) {
  const pkg = utils.parsePkgRef (packageRef);

  xLog.info (`Unpublish ${pkg.name || 'all'} package${pkg.name && 's'} ` +
             `on ${pkg.arch || 'all architectures'}.`);
  ...
  const distrib = /.*-src$/.test (pkg.name) ? 'sources' : def.distribution;
  wpkg.unpublish (pkg.name, pkg.arch, repository, distrib, callback);
};
